...
const randomRanges = {
    ...
    enableIntervalRandomization: false // Added this based on usage in randomizeTerrainSettings
};
...
let fadeRange: number = config.maxFadeDistance - config.minFadeDistance;

// --- Pre-create colors ---
let baseContourColor: THREE.Color = new THREE.Color(config.contourColor);
let fadeToBgColor: THREE.Color = new THREE.Color(config.backgroundColor);
...
// --- Terrain Generation ---
function generateTerrain(): THREE.Mesh {
...
// --- Create Terrain Border ---
// ... (no changes needed) ...
function createTerrainBorder(): void {
    if (terrainBorder) {
...
function generateContourLines(geometry: THREE.BufferGeometry): void {
...
function randomizeTerrainSettings(): void {
